...
import {
  buildStoreSegmentedImportSessionKey,
  deleteStoreSegmentedImportSession,
  loadStoreSegmentedImportPart,
  loadStoreSegmentedImportSession,
  saveStoreSegmentedImportPart,
  saveStoreSegmentedImportSession,
} from '@/lib/store-segmented-import';
...
const fetchBlobWithProgress = async (
  ...
  if (!response.body) {
    ...
    onChunk?.(blob.size, blob.size || null);
    return blob;
  }
  ...
  while (true) {
    ...
    if (value) {
      ...
      onChunk?.(receivedBytes, Number.isFinite(totalBytes) ? totalBytes : null);
    }
  }
  return new Blob(chunks, { type: 'application/octet-stream' });
};

const normalizeSegmentedManifest = (value: unknown): SegmentedImportManifest => {
  if (!value || typeof value !== 'object') {
    throw new Error('Invalid low-memory manifest.');
  }
  const manifest = value as SegmentedImportManifest;
  if (!manifest.catalogItemId || !manifest.bankId || !manifest.bankData || !Array.isArray(manifest.parts)) {
    throw new Error('Invalid low-memory manifest structure.');
  }
  return manifest;
};
...
const runSegmentedStoreImportPipeline = async (
  source: Extract<ImportBankSource, { kind: 'segmented-store' }>,
  ...
  const importBankName = source.fileName || `${source.catalogItemId}.bank`;
  ...
  const sessionKey = buildStoreSegmentedImportSessionKey(source.catalogItemId, source.variantId);
  const existingSession = await loadStoreSegmentedImportSession(sessionKey).catch(() => null);
  const totalParts = source.parts.length;
  const nowMs = Date.now();
  await saveStoreSegmentedImportSession({
    sessionKey,
    catalogItemId: source.catalogItemId,
    bankId: source.bankId,
    variantId: source.variantId,
    mode: 'low_memory_segmented',
    fileName: source.fileName || `${source.catalogItemId}.bank`,
    fileSizeBytes: source.fileSizeBytes ?? null,
    completedPartIndexes: existingSession?.completedPartIndexes || [],
    totalParts,
    sourceRevisionHash: null,
    createdAt: existingSession?.createdAt || nowMs,
    updatedAt: nowMs,
  });

  try {
    reportImportStage('Loading low-memory manifest...', 3, 'segmented-manifest');
    const manifestResponse = await fetch(source.manifest.downloadUrl, { cache: 'no-store' });
    if (!manifestResponse.ok) {
      throw new Error(`Low-memory manifest download failed (${manifestResponse.status})`);
    }
    const manifest = normalizeSegmentedManifest(await manifestResponse.json());
    if (manifest.catalogItemId !== source.catalogItemId || manifest.bankId !== source.bankId) {
      throw new Error('Low-memory manifest does not match the selected bank.');
    }
    importPadNames = Array.isArray(manifest.bankData?.pads)
      ? manifest.bankData.pads.map((pad: any) => String(pad?.name || 'Untitled Pad'))
      : [];
    includePadList = false;

    const completedPartIndexes = new Set<number>(existingSession?.completedPartIndexes || []);
    for (let partListIndex = 0; partListIndex < source.parts.length; partListIndex += 1) {
      const part = source.parts[partListIndex];
      if (!completedPartIndexes.has(part.partIndex)) {
        reportImportStage(
          `Downloading low-memory part ${part.partIndex + 1}/${totalParts}...`,
          Math.round(5 + ((partListIndex / Math.max(totalParts, 1)) * 35)),
          'segmented-download',
        );
        const blob = await fetchBlobWithProgress(part.downloadUrl, (receivedBytes, totalBytes) => {
          const ratio = totalBytes && totalBytes > 0 ? Math.min(1, receivedBytes / totalBytes) : 0;
          const phaseBase = 5 + ((partListIndex / Math.max(totalParts, 1)) * 35);
          const phaseSpan = 35 / Math.max(totalParts, 1);
          onProgress?.(Math.round(phaseBase + ratio * phaseSpan));
        });
        await saveStoreSegmentedImportPart({
          sessionKey,
        ...
        completedPartIndexes.add(part.partIndex);
        await saveStoreSegmentedImportSession({
          sessionKey,
          catalogItemId: source.catalogItemId,
          bankId: source.bankId,
          ...
          fileName: source.fileName || `${source.catalogItemId}.bank`,
          fileSizeBytes: source.fileSizeBytes ?? null,
          completedPartIndexes: Array.from(completedPartIndexes).sort((a, b) => a - b),
          totalParts,
          sourceRevisionHash: manifest.sourceAssetSha256 || null,
          createdAt: existingSession?.createdAt || nowMs,
          updatedAt: Date.now(),
        });
      }
    ...
    let bankData = manifest.bankData;
    let metadata: BankMetadata | null = (manifest.metadata as BankMetadata | null) || null;
    metadata = {
      ...(metadata || {
        password: false,
        transferable: true,
      ...
      bankId: source.bankId,
      catalogItemId: source.catalogItemId,
      entitlementToken: source.entitlementToken || metadata?.entitlementToken,
      entitlementTokenKid: source.entitlementTokenKid || metadata?.entitlementTokenKid,
      entitlementTokenIssuedAt: source.entitlementTokenIssuedAt || metadata?.entitlementTokenIssuedAt,
      entitlementTokenExpiresAt: source.entitlementTokenExpiresAt || metadata?.entitlementTokenExpiresAt,
    };

    const bankJsonText = JSON.stringify(bankData);
    ...
    const metadataBankId = metadata?.bankId || source.bankId || undefined;
    ...
    let resolvedBankName = String(bankData?.name || source.fileName || 'Bank');
    ...
    const signedAdminExportToken = typeof metadata?.adminExportToken === 'string'
      ...
      : '';
    if (signedAdminExportToken && bankJsonSha256) {
    ...
    const requiresSignedEntitlement = Boolean(
      isAdminBank &&
      metadataBankId &&
      (metadata?.catalogItemId || options?.preferredDerivedKey || source.derivedKey)
    );
    if (requiresSignedEntitlement && !signedEntitlementToken && metadata?.catalogItemId && userForAccess?.id) {
      ...
      if (accessMaterial?.entitlementToken) {
        metadata = {
          ...
          entitlementTokenKid: accessMaterial.entitlementTokenKeyId || metadata?.entitlementTokenKid,
    ...
    const resolvedSignedEntitlementToken = entitlementTokenFromOption || (typeof metadata?.entitlementToken === 'string' ? metadata.entitlementToken.trim() : '');
    ...
    reportImportStage('Preparing low-memory bank import...', 42, 'segmented-prepare');
    const maxSortOrder = currentBanks.length > 0 ? Math.max(...currentBanks.map((b) => b.sortOrder || 0)) : -1;
    ...
    const flushPendingBatchFiles = async () => {
      ...
      pendingBatchBytes = 0;
      await saveBatchBlobsToDB(items);
    };
      ...
      try {
        ...
        if (audioBlob) {
          ...
          const admissionMetadata =
            aggressiveAndroidImport || (trustedAudioBytes > 0 && inferredDurationMs > 0)
    ...
    const adaptiveTimeoutMs = aggressiveAndroidImport ? 120_000 : 90_000;
    const candidateKeys = Array.from(new Set([
      typeof options?.preferredDerivedKey === 'string' ? options.preferredDerivedKey.trim() : '',
      typeof source.derivedKey === 'string' ? source.derivedKey.trim() : '',
      SHARED_EXPORT_DISABLED_PASSWORD,
    ].filter(Boolean)));

    let processedPads = 0;
    for (const part of source.parts.sort((left, right) => left.partIndex - right.partIndex)) {
      const persistedPart = await loadStoreSegmentedImportPart(sessionKey, part.partIndex);
      if (!persistedPart?.blob) {
        throw new Error(`Missing low-memory part ${part.partIndex + 1}.`);
      }
      const file = new File([persistedPart.blob], `${source.fileName || source.catalogItemId}.part${part.partIndex + 1}.bank`, {
        type: 'application/octet-stream',
      });
      reportImportStage(`Importing low-memory part ${part.partIndex + 1}/${totalParts}...`, Math.round(45 + ((processedPads / Math.max(importPadNames.length, 1)) * 50)), 'segmented-import');
      const openResult = await workerClient.open(file, candidateKeys, adaptiveTimeoutMs);
      const partBankData = openResult.bankData as any;
      if (!partBankData || !Array.isArray(partBankData.pads)) {
        throw new Error(`Low-memory part ${part.partIndex + 1} is invalid.`);
      }
      for (let i = 0; i < partBankData.pads.length; i += concurrentPadCount) {
        const chunk = partBankData.pads.slice(i, i + concurrentPadCount);
        ...
        const extractedChunkItems = await workerClient.extractPadChunk(workerChunkDescriptors);
        const extractedByIndex = new Map<number, ImportWorkerPadChunkItem>();
        extractedChunkItems.forEach((item) => extractedByIndex.set(item.index, item));
        const chunkResults = await Promise.allSettled(
          chunk.map((padData: any, localIndex: number) =>
            processPad(padData, processedPads + localIndex, extractedByIndex.get(i + localIndex) || null),
          ),
        );
        for (const result of chunkResults) {
          if (result.status === 'fulfilled' && result.value) {
            newPads.push(result.value);
          }
          processedPads += 1;
          const progress = 45 + ((processedPads / Math.max(importPadNames.length || manifest.totalPads || 1, 1)) * 50);
    ...
    await flushPendingBatchFiles();
    await workerClient.dispose().catch(() => undefined);
    if (newPads.length === 0) {
      throw new Error('No valid pads found in low-memory import.');
    }
    ...
    await deleteStoreSegmentedImportSession(sessionKey).catch(() => undefined);
    reportImportStage('Low-memory import complete.', 100, 'segmented-complete');
    logImportActivity({
      status: 'success',
    ...
    const errorMessage = e instanceof Error ? e.message : 'Unknown segmented import error';
    reportImportStage(`Import failed: ${errorMessage}`, undefined, 'segmented-failed');
    logImportActivity({
      status: 'failed',
...
export const runImportBankPipeline = async (
  ...
  deps: ImportBankPipelineDeps
    ...
    if (isSegmentedStoreImportSource(source)) {
      return runSegmentedStoreImportPipeline(source, onProgress, options, deps);
    }
    if (
