...
create index if not exists idx_account_upgrade_requests_refund_status
  on public.account_upgrade_requests (status, is_refunded, created_at desc);

create or replace view public.v_admin_dashboard_revenue_totals
with (security_invoker = true)
as
with account_buyers as (
  select coalesce(nullif(email_normalized, ''), lower(trim(email))) as buyer_key
  from public.account_registration_requests
  where status = 'approved'
    and coalesce(nullif(email_normalized, ''), lower(trim(email))) <> ''
  union
  select coalesce(user_id::text, lower(trim(email))) as buyer_key
  from public.account_upgrade_requests
  where status = 'approved'
    and coalesce(user_id::text, lower(trim(email))) <> ''
)
select
  coalesce(
    (
  ...
  (
    ...
    coalesce(
      (
        select sum(coalesce(quote_price_php_snapshot, 0)::numeric)
        from public.account_upgrade_requests
        where status = 'approved'
          and coalesce(is_refunded, false) = false
      ),
...
account_source as (
  select
    ...
    coalesce(account_price_php_snapshot, 0)::numeric as amount_php,
    coalesce(nullif(email_normalized, ''), lower(trim(email))) as buyer_key
  from public.account_registration_requests
  union all
  select
    created_at,
    status,
    coalesce(is_refunded, false) as is_refunded,
    coalesce(quote_price_php_snapshot, 0)::numeric as amount_php,
    coalesce(user_id::text, lower(trim(email))) as buyer_key
  from public.account_upgrade_requests
),
account as (
  select
    ...
    coalesce(
      sum(
        case
          when status = 'approved' and is_refunded = false
            then amount_php
          else 0::numeric
        end
      ),
    ...
    count(
      distinct case
        when status = 'approved' and buyer_key <> '' then buyer_key
        else null
      end
